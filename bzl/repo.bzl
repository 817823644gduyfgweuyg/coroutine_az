...
def _tensorflow_serving_http_archive(ctx):
    ...
    strip_prefix = "serving-%s" % git_commit
    urls = [
        "https://mirror.bazel.build/github.com/tensorflow/serving/archive/%s.tar.gz" % git_commit,
        "https://github.com/tensorflow/serving/archive/%s.tar.gz" % git_commit,
    ]
...
tensorflow_serving_http_archive = repository_rule(
    implementation = _tensorflow_serving_http_archive,
    attrs = {
